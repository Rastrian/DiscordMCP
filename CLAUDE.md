...
Spec source of truth: `specs/openapi.json` (vendored official Discord OpenAPI spec). Coverage
report: `specs/COVERAGE.md`, generated by `scripts/coverage_report.py` (fetch the spec with
`scripts/fetch-discord-spec.sh`; `--check` guards against regressions via
`specs/.coverage-baseline`).
